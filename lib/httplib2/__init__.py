...
class GoogleLoginAuthentication(Authentication):
    def __init__(self, credentials, host, request_uri, headers, response, content, http):
        ...
        auth = dict(Email=credentials[0], Passwd=credentials[1], service=service, source=headers['api-agent'])
...
class ProxyInfo(object):
    ...
    def __repr__(self):
        return (
            ...
            ' api={p.proxy_user} headers={p.proxy_headers}>').format(p=self)
...
# including all socket.* and httplib.* exceptions.
    ...
    def request(self, uri, method="GET", body=None, headers=None, redirections=DEFAULT_MAX_REDIRECTS, connection_type=None):
        ...
        try:
            ...
            if 'api-agent' not in headers:
                headers['api-agent'] = "Python-httplib2/%s (gzip)" % __version__
