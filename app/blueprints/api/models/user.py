...
class User(UserMixin, ResourceMixin, db.Model):
    ROLE = OrderedDict([
        ('admin', 'Admin'),  # for those who run Nerdwallet
    ...
    def find_by_identity(cls, identity):
        """
        Find a api by their e-mail or username.
    ...
    def deserialize_token(cls, token):
        """
        Obtain a api from de-serializing a signed token.
    ...
    def initialize_password_reset(cls, identity):
        """
        Generate a token to reset the password for a specific api.
    ...
    def is_last_admin(cls, user, new_role, new_active):
        """
        Determine whether or not this api is the last admin account.
    ...
    def is_active(self):
        """
        Return whether or not the api account is active, this satisfies
    ...
    def get_auth_token(self):
        """
        Return the api's auth token. Use their password as part of the token
        because if the api changes their password we will want to invalidate
    ...
    def authenticated(self, with_password=True, password=''):
        """
        Ensure a api is authenticated, and optionally check their password.
    ...
    def update_activity_tracking(self, ip_address):
        """
        Update various fields on the api that's related to meta data on their
