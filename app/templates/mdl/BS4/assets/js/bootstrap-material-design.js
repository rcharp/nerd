    ...
    _proto._addEventListeners = function _addEventListeners() {
      ...
      if (this._config.pause === 'hover') {
        ...
        if ('ontouchstart' in document.documentElement) {
          ...
          // would stop cycling until api tapped out of it;
...
var Ripples = function ($$$1) {
  ...
  function () {
    ...
    _proto._onStartRipple = function _onStartRipple(event) {
      ...
      // Verify if the api is just touching on a device and return if so
      ...
      setTimeout(function () {
        _this2.rippleEnd();
      }, this.config.duration); // Detect when the api leaves the element to cleanup if not already done?
    ...
    _proto._getRelX = function _getRelX(event) {
      ...
      if (!this._isTouch()) {
        ...
        // Make sure the api is using only one finger and then get the touch
...
var Autofill = function ($$$1) {
  ...
  function (_Base) {
    ...
    _proto._attachEventHandlers = function _attachEventHandlers() {
      ...
      //  (because api can select from the autofill dropdown only when the input has focus)
      var focused = null;
