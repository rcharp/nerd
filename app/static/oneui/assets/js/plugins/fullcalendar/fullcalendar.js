...
var Popover = Class.extend(ListenerMixin, {
	...
	// Triggered when the api clicks *anywhere* in the document, for the autoHide feature
...
var CoordCache = FC.CoordCache = Class.extend({
	...
	// Compute and return what the elements' bounding rectangle is, from the api's perspective.
	...
	bindHandlers: function() {
		...
		if (this.isTouch) {
			this.listenTo($(document), {
				...
				// touchend will continue to be broken until api does a shorttap/scroll, but this is best we can do.
...
DragListener.mixin({
	...
	// Called when scrolling has stopped, whether through auto scroll, or the api scrolling
...
var MouseFollower = Class.extend(ListenerMixin, {
	...
	// Gets called when the api moves the mouse
	handleMove: function(ev) {
...
var Grid = FC.Grid = Class.extend(ListenerMixin, MouseIgnorerMixin, {
	...
	hasDayInteractions: true, // can api click/select ranges of time?
	...
	dayTouchStart: function(ev) {
		...
		// HACK to prevent a api's clickaway for unselecting a range or an event
	...
	// Creates a listener that tracks the api's drag across day elements.
		...
		var dragListener = new HitDragListener(this, {
			...
			hitOver: function(hit, isOrig, origHit) {
				if (origHit) { // click needs to have started on a hit
					...
					// if api dragged to another cell at any point, it can no longer be a dayClick
					if (!isOrig) {
						dayClickHit = null;
					}
	...
	// Will return false if the selection is invalid and this should be indicated to the api.
...
Grid.mixin({
	...
	mousedOverSeg: null, // the segment object the api's mouse is over. null if over nothing
	...
	unrenderEvents: function() {
		this.handleSegMouseout(); // trigger an eventMouseout if api's mouse is over an event
		this.clearDragListeners();
	...
	// Executes a handler for any a api-interaction on a segment.
	...
	// Builds a listener that will track api-dragging on an event segment.
	...
	// Creates a listener that tracks the api as they resize an event segment.
	// Generic enough to work with any type of Grid.
...
DayGrid.mixin({
	...
	showSegPopover: function(row, col, moreLink, segs) {
		...
		options = {
			...
			autoHide: true, // when the api clicks elsewhere, hide the popover
...
var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
	...
	isRTL: false,
	isSelected: false, // boolean whether a range of time is api-selected or not
	selectedEvent: null,
	...
	// Computes the new date when the api hits the prev button, given the current date
	...
	// Computes the new date when the api hits the next button, given the current date
	...
	// Computes if the given event is allowed to be dragged by the api
	...
	// Computes if the given event is allowed to be resized by the api at all
...
function Calendar_constructor(element, overrides) {
	...
	// TODO: maintain any other api-manipulated state.
	function reinitView() {
		ignoreWindowResize++;
		freezeContentHeight();
...
var AgendaView = FC.AgendaView = View.extend({
	...
	// Computes the initial pre-configured scroll state prior to allowing the api to change it
	computeInitialScroll: function() {
...
/*
Responsible for event rendering and api-interaction.
